...
def run_q9_top10_rerank(csv_path_list, result_csv_path_list, reviews_path, model_path, embedding_path, n_device, sql_path, movies_path, reviews_path):
    reviews = pd.read_csv(reviews_path)
    filtered_reviews = reviews[
            reviews["id"] == "ant_man_and_the_wasp_quantumania"
        ]
...
def run_recompute(path, val, tour, reviews_path, model_path, embedding_path, n_device, sql_path, movies_path, reviews_path):
    csv_path = []
    result_path = []
    i = 1
    while i <= 10:

...
def gen_ground_truth(sql_path, movies_path, reviews_path):
    ...
    conn.register('Reviews', pd.read_csv(movies_path))
...
df, metric = run_q9_rz_sort(int(sys.argv[2]), f'rzt{int(sys.argv[2])}_{int(sys.argv[3])}.csv', sys.argv[4], '../../../../models_and_benchmarks/rankzephyr', "../../../../models_and_benchmarks/em_model", int(sys.argv[1]))
#df, metric = run_q9_rz_tour(int(sys.argv[1]), sys.argv[2])
#df, metric = run_q9_map(sys.argv[3], sys.argv[4])
gt = gen_ground_truth('../../../../models_and_benchmarks/movies/Q9.sql', sys.argv[5], sys.argv[4])
evaluate_results(gt, df, metric, f'q9_t{int(sys.argv[2])}_{int(sys.argv[3])}.csv')

...
def run_q9_rz_sort(w, label, reviews_path, model_path, embedding_path, n_device):
    ...
    test = MultiPivot_sort(model_path=model_path, n_devices=n_device, window_size = w)
    start_time = time.perf_counter()
    result = test.sem_sort(query="Which review shows the most positive sentiment about the movie?", documents=formatted_documents, pivots=2, group_size=2, pivot_selection_method= 'embedding', embedding_path= embedding_path)
...
def gen_ground_truth(sql_path, movies_path, reviews_path):
    sql_path = sql_path
    ...
    conn.register('Movies', pd.read_csv(movies_path))
    conn.register('Reviews', pd.read_csv(reviews_path))
...
def run_q9_rz_tour(w, label, reviews_path, model_path, n_device):
    reviews = pd.read_csv(reviews_path)
    filtered_reviews = reviews[
            reviews["id"] == "ant_man_and_the_wasp_quantumania"
        ]
    ...
    for i in range(len(ids)):
        formatted_documents.append(f"{ids[i]}:{text[i]}:{i}")
    test = MultiPivot_tour(model_path=model_path, n_devices=n_device, window_size = w)
    start_time = time.perf_counter()
...
def run_q9_map(name, label, reviews_path):
    import lotus
...
def run_q9_top10_rerank(csv_path_list, result_csv_path_list, reviews_path, model_path, embedding_path, n_device, sql_path, movies_path, review_path):
    reviews = pd.read_csv(reviews_path)
    filtered_reviews = reviews[
            reviews["id"] == "ant_man_and_the_wasp_quantumania"
        ]
    if len(filtered_reviews) == 0:
        ...
        return pd.DataFrame(columns=["reviewId", "reviewScore"])
    test = MultiPivot_sort(model_path=model_path, n_devices=n_device, window_size = 2)
    ids = list(filtered_reviews['reviewId'])
    text = list(filtered_reviews['reviewText'])
    label_id = 1
    result_path = 0
    for csv in csv_path_list:
        ...
        for t in top_10:
            ...
            i = i + 1
        start_time = time.perf_counter()
        result = test.sem_sort(query="Which review shows the most positive sentiment about the movie?", documents=formatted_reviews, pivots=1, group_size=1, pivot_selection_method= 'embedding', embedding_path= embedding_path)
        end_time = time.perf_counter()
        formatted_other = []
        for i in data_ids:
        ...
        gt = gen_ground_truth(sql_path, movies_path, reviews_path)
...
def run_recompute(path, val, tour, reviews_path, model_path, embedding_path, n_device, sql_path, movies_path, review_path):
    csv_path = []
    result_path = []
    i = 1
    while i <= 10:
        if tour:
            ...
            result_path.append(temp)
        i = i + 1
    run_q9_top10_rerank(csv_path, result_path, reviews_path, model_path, embedding_path, n_device, sql_path, movies_path, reviews_path)
    return

import sys
#generate_data('../../../../models_and_benchmarks/movies')
#df, metric = run_q9_rz_sort(int(sys.argv[4]), sys.argv[1], sys.argv[5], sys.argv[6], sys.argv[7], int(sys.argv[3]))
df, metric = run_q9_rz_tour(20, f'rzt20_{int(sys.argv[2])}.csv', sys.argv[3], '../../../../models_and_benchmarks/rankzephyr', int(sys.argv[1]))
#df, metric = run_q9_map(sys.argv[3], sys.argv[4])
gt = gen_ground_truth('../../../../models_and_benchmarks/movies/Q9.sql', sys.argv[4], sys.argv[3])
evaluate_results(gt, df, metric, f'q9t_20_{int(sys.argv[2])}.csv')

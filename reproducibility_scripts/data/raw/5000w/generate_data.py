...
def run_test_unsorted(
    ...
    test = MultiPivot(model_path=rankzephyr_path, n_devices=device_num, window_size=w)
    ...
    while q < stop:
        if cutoff != None:
            temp = test.embedding_filter(query=query_doc[q], documents=document_set, cutoff=cutoff, embedding_path= embedding_path)
        print(str(q) + "/" + str(len(query_doc)) + ": " + str(query_doc[q]))
        start_time = time.perf_counter()
        result = test.top_k(query=query_doc[q], documents=document_set, top_k=10, pivots=1, group_size=1, pivot_selection_method= "embedding", tournament_filter=[False, 200, 100, 10], embedding_path= embedding_path,
...
#Confirm Later
import sys
run_test_unsorted('../../../../models_and_benchmarks/scifact', '../../../../models_and_benchmarks/rankzephyr', "../../../../models_and_benchmarks/em_model", int(sys.argv[1]), 25, int(sys.argv[2]))

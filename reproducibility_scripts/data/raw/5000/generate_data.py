...
def run_test_unsorted(
    ...
    while q < stop:
        ...
        result = test.top_k(query=query_doc[q], documents=document_set, top_k=10, pivots=p, group_size=x, pivot_selection_method= selection_method, tournament_filter=[False, 518, 100, 10], embedding_path= embedding_path,
                            embedding_filter=[False, 200], early_stop=False)
...
import sys
run_test_unsorted('../../../../models_and_benchmarks/scifact', '../../../../models_and_benchmarks/rankzephyr', "../../../../models_and_benchmarks/em_model", int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]), "embedding", 25)

...
import math
'''
    See gensemsort.py for specifics of methods. This is the same, but takes the RankZephyr PLM class rather than the GenericRankLM version.
'''
class MultiPivot_sort():
    call_count = 0
    def __init__(
        self,
        model_path: str, 
    ...
    def pivot_selection(
        ...
        if len(pivot_list) == 1:
            return pivot_list
        MultiPivot_sort.call_count = MultiPivot_sort.call_count + 1
    ...
    def organize_documents(
        ...
        MultiPivot_sort.call_count = MultiPivot_sort.call_count + len(organized_documents)
        counter = 0
        batched_documents = []
        current_batch = []
